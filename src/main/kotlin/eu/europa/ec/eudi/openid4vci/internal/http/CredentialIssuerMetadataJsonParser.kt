...
@Serializable
@SerialName(FORMAT_SE_TLV_VC)
private data class SeTlvVcCredentialTO(
    @SerialName("format") @Required override val format: String = FORMAT_SE_TLV_VC,
    @SerialName("scope") override val scope: String? = null,
    @SerialName("cryptographic_binding_methods_supported")
    override val cryptographicBindingMethodsSupported: List<String>? = null,
    @SerialName("credential_signing_alg_values_supported")
    ...
    init {
        require(format == FORMAT_SE_TLV_VC) { "invalid format '$format'" }
    }

    override fun toDomain(): SeTlvVcCredential {
        val bindingMethods = cryptographicBindingMethodsSupported
            ?.map { cryptographicBindingMethodOf(it) }
            ?: emptyList()
        val display = display?.map { it.toDomain() } ?: emptyList()
