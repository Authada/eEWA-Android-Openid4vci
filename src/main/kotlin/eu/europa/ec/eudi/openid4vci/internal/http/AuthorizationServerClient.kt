...
import eu.europa.ec.eudi.openid4vci.internal.dpop
import eu.europa.ec.eudi.openid4vci.internal.http.PushedAuthorizationNonceRequestResponseTO.Success
...
/**
 * Sealed hierarchy of possible responses to a Pushed Authorization Nonce Request.
 */
internal sealed interface PushedAuthorizationNonceRequestResponseTO {

    @Serializable
    data class Success(
        @SerialName("c_nonce") val cnonce: String,
        @SerialName("c_nonce_expires_in") val expiresIn: Long,
    ) : PushedAuthorizationNonceRequestResponseTO
}
...
internal sealed interface TokenResponseTO {
    ...
    fun tokensOrFail(): TokenResponse =
        when (this) {
            ...
            is Failure -> throw AccessTokenRequestFailed(
                error,
                errorDescription,
...
internal class AuthorizationServerClient(
    ...
    suspend fun submitParOrCreateAuthorizationRequestUrl(
        ...
        if (supportsPar) {
            ...
            authorizationRequestUrl(scopes, credentialsConfigurationIds, state, issuerState, claims)
        }
    ...
    private suspend fun Builder.clientAttestation() {
        getClientAttestationIfNeeded()?.let {
            customParameter("client_assertion_type", ClientAttestationProvider.TYPE_URN)
            customParameter(
                "client_assertion",
                it.clientAttestation,
            )
        }
    }
    ...
    fun authorizationRequestUrl(
        ...
        val authorizationRequest = AuthorizationRequest.Builder(ResponseType.CODE, clientID).apply {
            ...
            if (credentialsAuthorizationDetails.isNotEmpty()) {
                authorizationDetails(credentialsAuthorizationDetails.map { toNimbus(it, claims) })
            }
        }.build()
    ...
    suspend fun requestAccessTokenAuthFlow(
        ...
        val params = TokenEndpointForm.authCodeFlow(
            ...
            pkceVerifier = pkceVerifier,
            clientAttestationType = getClientAttestationIfNeeded(),

        )
        requestAccessToken(params).tokensOrFail()
    }

    private suspend fun getClientAttestationIfNeeded() = if (authorizationServerMetadata.supportsClientAttestation()) {
        config.clientAttestationProvider?.let {
            val nonce = fetchClientAttestationNonce()
            it.getAttestation(nonce.cnonce)
        } ?: throw PushedAuthorizationRequestFailed("Client attestation required", null)
    } else null

    private suspend fun fetchClientAttestationNonce() = ktorHttpClientFactory().use { client ->
        val url = authorizationServerMetadata.pushedAuthorizationRequestEndpointURI.toURL()
        val response = client.get(url.toString()) {
            accept(Application.Json)
        }
        if (response.status.isSuccess()) response.body<Success>()
        else null
    } ?: throw PushedAuthorizationRequestFailed("Fetching client attestation nonce failed", null)
...
internal object TokenEndpointForm {
    ...
    fun authCodeFlow(
        ...
        redirectionURI: URI,
        pkceVerifier: PKCEVerifier,
        clientAttestationType: ClientAttestationType? = null,
        ...
        clientAttestationType?.let {
            put(CLIENT_ASSERTION_PARAM, clientAttestationType.clientAttestation)
            put(CLIENT_ASSERTION_TYPE_PARAM, ClientAttestationProvider.TYPE_URN)
        }
    }.toMap()
